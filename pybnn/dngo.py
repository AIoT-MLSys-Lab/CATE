...
class DNGO(BaseModel):
    ...
    def train(self, X, y, do_optimize=True):
        ...
        self.network = Net(n_inputs=features, n_units=self.n_units).cuda()

        optimizer = optim.AdamW(self.network.parameters(),
                          lr=self.init_learning_rate, eps=1e-8)

        self.loss = WeightedLoss(weight=5.0)
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in self.iterate_minibatches(self.X, self.y,
                ...
                inputs = torch.Tensor(batch[0]).cuda()
                targets = torch.Tensor(batch[1]).cuda()
        ...
        # Design matrix
        self.Theta = self.network.basis_funcs(torch.Tensor(self.X).cuda()).cpu().data.numpy()
    ...
    def predict(self, X_test):
        ...
        theta = self.network.basis_funcs(torch.Tensor(X_).cuda()).cpu().data.numpy()
